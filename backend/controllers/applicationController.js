...
export const employerGetAllApplications = catchAsyncErrors(async (req, res, next) => {
  ...
  const { _id } = req.user;
  const applications = await Application.find({ "employerID.user": _id }).populate('job', 'title company'); 
  res.status(200).json({
    success: true,
    applications,
  });
...
// Fetch all applications for a jobseeker
export const jobseekerGetAllApplications = catchAsyncErrors(async (req, res, next) => {
  ...
  const { _id } = req.user;
  const applications = await Application.find({ "applicantID.user": _id }).populate('job', 'title company');
  res.status(200).json({
    success: true,
    applications,
  });
